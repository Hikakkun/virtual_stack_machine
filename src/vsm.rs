...
impl Vsm {
    ...
    pub fn read_code(&mut self, file_path :&str) -> io::Result<()> {
        ...
        for line in reader.lines() {
            ...
            let command_remove_comments = Vsm::remove_comments(&command);
            let command_parse = command_remove_comments.split_whitespace().collect::<Vec<_>>();
            ...
            if !command_parse.is_empty() {
                ...
                if operation_code_result.is_err() {
                    ...
                    return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid OperationCode"));
                }
                ...
                if let Some(operand_size) = self.operand_size_map.get(&operation_code) {
                    ...
                    if operand_size +1 != command_parse.len() {
                        eprintln!("Number of operands expected for OperationCode does not match: {} {} {}", operation_str, command_parse.len()-1, operand_size);
                        return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid OperationCode"));
                    }

                    let operand0 = if command_parse.len() > 1 {command_parse[1].parse::<i32>().map(Some)} else {Ok(None)};
                    let operand1 = if command_parse.len() > 2 {command_parse[2].parse::<i32>().map(Some)} else {Ok(None)};
                    
                    self.append_instruction(operation_code, operand0.unwrap(), operand1.unwrap());


                } else {
                    eprintln!("OperationCode operand_size is not defined: {}", operation_str);
                    return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid OperationCode"));
                }
    ...
    pub fn write_code(&self, file_path :&str ) -> io::Result<()>{
        let mut file = File::create(file_path)?;
        self.code.iter().try_for_each(|instruction| writeln!(file, "{}", instruction))?;

        Ok(())
    }
    pub fn append_instruction(&mut self, operation_code : OperationCode, operand0 : Option<i32>, operand1 : Option<i32>){
        self.code.push(Instruction{operation_code: operation_code, operand : [operand0, operand1]});
    }

    pub fn set_instruction(&mut self, index :usize,operation_code : OperationCode, operand0 : Option<i32>, operand1 : Option<i32>){
        self.code[index] = Instruction{operation_code: operation_code, operand : [operand0, operand1]};
    }
    pub fn print_code(&self){
        self.code.iter().for_each(|instruction| println!("{}", instruction));
    }
}